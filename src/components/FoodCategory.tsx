  ...
  return (
    <section className="bg-black text-white py-16 px-6">
      <div className="container mx-auto text-center">
        {/* Heading */}
        <h1 className="md:text-4xl text-2xl font-normal text-[#FF9F0D] font-greatVibes">
          Food Category
        </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {items.map((item) => (
            <div key={item.id} className="relative group">
              ...
              <div className="absolute bottom-0 left-0 w-full bg-black bg-opacity-50 text-white text-lg md:text-xl font-semibold p-4 rounded-b-lg">
                {item.label}
              </div>
            </div>
          ))}
