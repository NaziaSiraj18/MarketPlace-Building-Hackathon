    ...
    <section className='bg-black px-8 sm:px-16 lg:px-[135px] text-white py-12'>
      <div className="container mx-auto">
        ...
        <div className="flex justify-center mb-8">
          <ul className="flex flex-wrap justify-center sm:justify-start gap-4 sm:gap-6">
        ...
        /*  <div className="hidden lg:block w-1/6 md:w-[30%]">
            <Image src="/main-menu.png" alt="Menu Decor" width={300} height={300} className="object-cover"/>
          </div>

          {/* Right Section: Menu Items */
     /*     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-8 flex-grow">
            {menuItems.map((item, index) => (
              <div key={index} className="flex flex-col md:flex-row rounded-lg overflow-hidden shadow-lg border border-gray-700">
    ...
    <section className='bg-black px-8 sm:px-16 lg:px-[135px] text-white py-12'>
      <div className="container mx-auto">
        ...
        <div className="flex justify-center mb-8">
          <ul className="flex flex-wrap justify-center sm:justify-start gap-4 sm:gap-6">
            {['Breakfast', 'Lunch', 'Dinner', 'Dessert', 'Drink', 'Snack', 'Soups'].map((item, index) => (
              <li key={index} className="cursor-pointer hover:text-yellow-500 transition duration-300 ease-in-out transform hover:scale-105">
                {item}
              </li>
            ))}
          </ul>
        ...
        <div className='flex flex-col lg:flex-row mt-5'>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-8 flex-grow">
            {menuItems.map((item, index) => (
              <div key={index} className="flex flex-col md:flex-row rounded-lg overflow-hidden shadow-lg border border-gray-700 transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-xl">
                <div className="w-full md:w-1/3">
                  <Image
                    src={item.image}
                    alt={item.name}
